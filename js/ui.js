...
export function renderNavbar() {
  ...
  <aside id="mobile-drawer" class="fixed right-0 top-0 z-[70] hidden h-full w-[86%] max-w-sm overflow-y-auto custom-scrollbar bg-royalGreen px-5 pb-8 pt-5 text-ivory shadow-premium lg:hidden">
    ...
    <nav class="mt-5 grid gap-1 font-ui text-lg font-semibold">
      ${[['Home','index.html'],['Kilimanjaro','kilimanjaro.html'],['Safaris','safaris.html'],['Day Trips','day-trips.html'],['Destinations','destinations.html'],['Blogs','blog.html'],['About','about.html'],['Contact','contact.html'],['Connect Expert','connect-expert.html'],['Summit Readiness','summit-readiness.html'],['Book Now','booking.html'],['Client Portal','client-portal.html'],['Admin','admin.html'],['Booking Policy','booking-policy.html'],['Payment Policy','payment-policy.html'],['Refund Policy','refund-policy.html'],['Privacy Policy','privacy-policy.html']].map(([n,h])=>`<a href="${h}" class="mobile-nav-link rounded-xl px-3 py-3 hover:bg-white/10">${n}</a>`).join('')}
  ...
  const openMenu = () => { drawer?.classList.remove("hidden"); backdrop?.classList.remove("hidden"); document.body.classList.add("overflow-hidden"); toggle?.setAttribute("aria-expanded", "true"); };
  const closeMenu = () => { drawer?.classList.add("hidden"); backdrop?.classList.add("hidden"); document.body.classList.remove("overflow-hidden"); toggle?.setAttribute("aria-expanded", "false"); };
